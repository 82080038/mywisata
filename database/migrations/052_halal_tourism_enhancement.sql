-- Migration 052: Wisata Halal Enhancement
-- This migration adds features for Halal Tourism
-- Date: 2026-07-18
-- Purpose: Prayer room locator, qibla direction, halal packages
...
CREATE TABLE IF NOT EXISTS prayer_rooms (
    ...
    facilities JSON, -- ["wudu_area", "separate_entrance", "prayer_mats", "quran", "ablution_facilities"]
    prayer_times JSON, -- {"fajr": "05:00", "dhuhr": "12:00", "asr": "15:30", "maghrib": "18:00", "isha": "19:30"}
...
CREATE TABLE IF NOT EXISTS halal_packages (
    ...
    inclusions TEXT, -- Comma-separated list
    exclusions TEXT,
...
CREATE TABLE IF NOT EXISTS halal_package_itinerary (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (package_id) REFERENCES halal_packages(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS halal_package_bookings (
    ...
    dietary_requirements JSON, -- ["no_beef", "no_seafood", "vegetarian", "allergies"]
...
-- Add halal-related columns to existing tables
SET @dbname = DATABASE();

-- Add to destinations
SET @tablename = 'destinations';
SET @columnname = 'has_prayer_room';
...
SET @columnname = 'qibla_direction';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = @dbname AND table_name = @tablename AND column_name = @columnname) > 0,
  'SELECT 1',
  CONCAT('ALTER TABLE ', @tablename, ' ADD COLUMN ', @columnname, ' DECIMAL(5, 2) NULL')
));
PREPARE alterIfNotExists FROM @preparedStatement;
EXECUTE alterIfNotExists;
DEALLOCATE PREPARE alterIfNotExists;

SET @columnname = 'prayer_times_available';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = @dbname AND table_name = @tablename AND column_name = @columnname) > 0,
  'SELECT 1',
  CONCAT('ALTER TABLE ', @tablename, ' ADD COLUMN ', @columnname, ' BOOLEAN DEFAULT FALSE')
...
SET @columnname = 'prayer_room_available';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = @dbname AND table_name = @tablename AND column_name = @columnname) > 0,
  'SELECT 1',
  CONCAT('ALTER TABLE ', @tablename, ' ADD COLUMN ', @columnname, ' BOOLEAN DEFAULT FALSE')
...
SET @columnname = 'halal_certified';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = @dbname AND table_name = @tablename AND column_name = @columnname) > 0,
  'SELECT 1',
  CONCAT('ALTER TABLE ', @tablename, ' ADD COLUMN ', @columnname, ' BOOLEAN DEFAULT FALSE')
...
SET @columnname = 'halal_certification_number';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = @dbname AND table_name = @tablename AND column_name = @columnname) > 0,
  'SELECT 1',
